...
template <typename T> vector<T> dijkstra(Graph &G, int start) {
...
public:
    ...
    for (int day = 1; day <= input_.D_; day++) {
      ...
      if (vertexOnProcess.size() > 0) {
        ...
        while (itr != vertexOnProcess.end()) {
          auto u = *itr;
          fillEdgeOfVertex(vertexOnProcess, vertexEdgeIndex, isProcessed, input_.halfG_[u].size(),
                           u, day, rem);
      ...
      if (rem > 0) {
        ...
        if (remainedVertex.size()) {
          ...
          if (remainedVertex.find(u) != remainedVertex.end())
            remainedVertex.erase(u);
          if (input_.halfG_[u].size())
            vertexOnProcess.push_back(u);
  ...
  void processRandomEdge(vector<bool> &isEdgeProcessed, vector<vector<int>> &edgeSetPerDay,
    ...
    if (notProcessedEdge.find(e) != notProcessedEdge.end())
      notProcessedEdge.erase(e);
  }
  ...
  int countUnreachableVertexNumber(const vector<vector<vector<bool>>> &cEdgeGraph) {
    ...
    for (int day = 0; day < input_.D_; day++) {
      for (int u = 0; u < input_.N_; u++) {
        if (!isReachableVertex(cEdgeGraph, day, u))
  ...
  ll computeSecondItem(const vector<vector<int>> &edgeSetPerDay, ll averageUsedEdgeNumMulWeight,
    ...
    for (int day = 0; day < input_.D_; day++) {
      ...
      res += (averageUsedEdgeNumMulWeight - sum) / 100000 * (averageUsedEdgeNumMulWeight - sum) /
             1000000;
    }
    return res;
  }
  ...
  ll computeSimulatedAnnealingScore(vector<vector<vector<bool>>> &cEdgeInGraph,
    ...
    ll l2DistUsedEdgeNumMulWeight =
        computeSecondItem(edgeSetPerDay, averageUsedEdgeNumMulWeight,
    averageUsedEdgeNum);
    */
    ...
    */
    ...
    /*
return -unreachableVertexCount * 10000000000 - alpha *
l2DistUsedEdgeNumMulWeight; return -unreachableVertexCount * 10000000000 - alpha
* l2DistUsedEdgeNum;
    */
  }
  ...
  void createInitialOutput() {
    ...
    for (int i = 0; i < input_.M_; i++)
      notProcessedEdge.insert(i);
    for (int day = 1; day <= input_.D_; day++) {
      int rem = (input_.M_ + input_.D_ - 1) / input_.D_;
      while (rem > 0 && !notProcessedEdge.empty()) {
        processRandomEdge(isEdgeProcessed, edgeSetPerDay, notProcessedEdge, cEdgeInGraph, day);
        rem--;
      }
    }
    ...
    cerr << "before compute simulated annealing score: "
         << computeSimulatedAnnealingScore(cEdgeInGraph, edgeSetPerDay, sumOfDistanceTwoEdgesPerDay,
    ...
    simulatedAnnealing(cEdgeInGraph, edgeSetPerDay, sumOfDistanceTwoEdgesPerDay, averageUsedEdgeNum,
  ...
  void updateSumOfDistanceTwoEdgesPerDay(vector<vector<int>> &edgeSetPerDay,
                                         vector<double> &sumOfDistanceTwoEdgesPerDay, int dayIndex1,
                                         int dayIndex2, int edgeIndex1, int edgeIndex2) {
  ...
  template <typename T> vector<T> dijkstraDay0(Graph &G, vector<int> &prev, int start) {
